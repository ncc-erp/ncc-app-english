...
import { createLaunchToken, verifyLaunchToken } from '../lib/auth/launch-token';
...
assert.notEqual(verifyLaunchToken(createLaunchToken(base, 5))!.jti, payload!.jti);

// 3. Tampered payload is rejected
const [data, sig] = token.split('.');
const forged = Buffer.from(JSON.stringify({ ...payload, mezonId: 'someone-else' })).toString('base64url');
...
const legacy = Buffer.from(JSON.stringify({ ...base, exp: Date.now() + 60_000 })).toString('base64url');
const legacySig = crypto.createHmac('sha256', process.env.SESSION_SECRET!).update(legacy).digest('base64url');
