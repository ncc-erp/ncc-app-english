	...
	if (loading) {
		return (
			<div className='min-h-screen bg-slate-50 text-slate-900 flex flex-col justify-center items-center'>
				...
				<p className='text-slate-600 font-medium'>{t('ielts.result.loadingAttempt')}</p>
			</div>
		);
	}
				...
				<main className='flex-1 max-w-2xl mx-auto px-4 py-20 w-full flex flex-col items-center justify-center text-center space-y-6'>
					{rescoring ? (
						<>
							...
							<div className='space-y-2'>
								...
								<h1 className='text-2xl sm:text-3xl font-extrabold text-slate-900'>{t('ielts.result.evaluatingHeading')}</h1>
								<p className='text-sm text-slate-600 max-w-md mx-auto leading-relaxed'>{t('ielts.result.evaluatingDescription')}</p>
							</div>
						</>
					) : (
						<>
							...
							<div className='space-y-2'>
								...
								<h1 className='text-3xl font-extrabold text-slate-900'>{t('ielts.result.pendingHeading')}</h1>
								<p className='text-sm text-slate-600 max-w-md mx-auto leading-relaxed'>{t('ielts.result.pendingDescription')}</p>
							</div>
						</>
					)}

					{error && (
						<div className='p-4 bg-rose-50 border border-rose-200 text-rose-700 rounded-2xl text-xs font-medium max-w-md w-full'>{error}</div>
					)}
					...
					<div className='flex flex-col sm:flex-row items-center gap-4 pt-2'>
						...
						>
							...
							<span>{rescoring ? t('ielts.result.rescoreAnalyzingLabel') : t('ielts.result.rescoreButtonLabel')}</span>
						</button>
				...
				<div className='relative overflow-hidden bg-gradient-to-br from-amber-500 via-amber-600 to-purple-700 text-white rounded-3xl p-8 md:p-12 shadow-xl'>
					<div className='flex flex-col md:flex-row items-center justify-between gap-8'>
						<div className='space-y-3 text-center md:text-left'>
							<div className='flex flex-wrap items-center justify-center md:justify-start gap-2'>
								...
								>
									...
									<span>{rescoring ? t('ielts.result.rescoreHeroAnalyzingLabel') : t('ielts.result.rescoreButtonLabel')}</span>
								</button>
							</div>
						...
						<div className='flex flex-col items-center justify-center p-6 bg-white/10 backdrop-blur-md border border-white/20 rounded-3xl min-w-[200px] shadow-2xl shrink-0'>
							<div className='text-xs uppercase tracking-widest font-bold text-amber-200'>{t('ielts.result.overallBandLabel')}</div>
					...
					<div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in'>
						<div className='bg-white border border-slate-200 rounded-3xl p-6 md:p-8 max-w-md w-full shadow-2xl space-y-6'>
							<div className='flex items-center gap-3'>
								...
								<div>
									<h3 className='text-lg font-bold text-slate-900'>{t('ielts.result.modalHeading')}</h3>
									<p className='text-xs text-slate-500 font-medium'>{t('ielts.result.modalSubtitle')}</p>
								</div>
							</div>

							<p className='text-sm text-slate-600 leading-relaxed font-medium'>{t('ielts.result.modalBody')}</p>
