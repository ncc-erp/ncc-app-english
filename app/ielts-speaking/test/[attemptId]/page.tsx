	...
	const handleFinishExam = async () => {
		try {
			...
			if (contentType.includes('application/json')) {
				...
				throw new Error(text || t('ielts.common.nonJsonResponse', { status: res.status }));
			}
	...
	if (loading) {
		return (
			<div className='min-h-screen bg-slate-50 text-slate-900 flex flex-col justify-center items-center'>
				...
				<p className='text-slate-600 font-medium'>{t('ielts.test.preparingEnvironment')}</p>
			</div>
		);
	}
		...
		<div className='min-h-screen bg-slate-50 text-slate-900 flex flex-col font-sans'>
			...
			<main className='flex-1 max-w-5xl mx-auto px-4 py-8 w-full'>
				...
				<div className='bg-white border border-slate-200 rounded-2xl p-4 mb-8 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm'>
					<div>
						<div className='text-xs uppercase tracking-wider font-bold text-purple-600'>{t('ielts.test.badge')}</div>
						...
						<div className='flex items-center gap-2 bg-slate-100 p-1.5 rounded-xl border border-slate-200'>
							<div
								className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-default select-none ${
									currentPart === 'part1' ? 'bg-purple-600 text-white shadow-sm' : 'text-slate-400 bg-slate-200/60'
								}`}
							>
							...
							<div
								className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-default select-none ${
									currentPart === 'part2' ? 'bg-amber-500 text-slate-950 shadow-sm' : 'text-slate-400 bg-slate-200/60'
								}`}
							>
							...
							<div
								className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-default select-none ${
									currentPart === 'part3' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-400 bg-slate-200/60'
								}`}
							>
					...
					<div className='space-y-6'>
						<div className='bg-purple-50/80 border border-purple-200 rounded-2xl p-6 flex items-center justify-between'>
							<div>
								...
								<h2 className='text-xl font-bold text-slate-900'>{t('ielts.test.part1Heading')}</h2>
								<p className='text-sm text-slate-600 mt-1'>{t('ielts.test.part1Instructions')}</p>
							</div>
						...
						<div className='bg-white border border-slate-200 rounded-2xl p-6 md:p-8 space-y-6 shadow-sm'>
							<div className='flex items-start justify-between gap-4'>
								<div>
									...
									<h3 className='text-xl font-extrabold text-slate-900 mt-1'>{currentP1Question.question_text}</h3>
								</div>
							...
							<AudioRecorder
								...
								onAudioRecorded={(url, transcript, duration, audioStoragePath) =>
									handleAudioRecorded(currentP1Question.id, 'part1', url, transcript, duration, audioStoragePath)
								}
				...
				{currentPart === 'part2' && (
					<div className='space-y-6'>
						...
						<div className='bg-white border border-amber-200 rounded-2xl p-6 md:p-8 shadow-sm'>
							<h3 className='text-xs font-bold text-amber-700 uppercase tracking-wider mb-3'>{t('ielts.test.cueCardHeading')}</h3>
						...
						{part2Step === 'prep' && (
							<div className='space-y-6'>
								...
								<div className='bg-white border border-slate-200 rounded-2xl p-6 shadow-sm'>
									<label className='block text-sm font-bold text-slate-800 mb-2'>{t('ielts.test.notesLabel')}</label>
									<textarea
										value={part2Notes}
							...
							<div className='space-y-6'>
								...
								<div className='bg-amber-50/70 border border-amber-200 rounded-2xl p-6 shadow-sm'>
									<div className='flex items-center gap-2 text-xs font-bold text-amber-800 uppercase tracking-wider mb-2'>
										...
										<span>{t('ielts.test.notesReferenceLabel')}</span>
									</div>
									<div className='bg-white border border-amber-200/80 rounded-xl p-4 text-slate-800 text-sm font-medium whitespace-pre-wrap min-h-[80px]'>
										{part2Notes.trim() ? part2Notes : <span className='text-slate-400 italic'>{t('ielts.test.noNotesPlaceholder')}</span>}
									</div>
								</div>

								<div className='bg-white border border-slate-200 rounded-2xl p-6 space-y-4 shadow-sm'>
									<h3 className='text-lg font-bold text-slate-900'>{t('ielts.test.recordSpeechHeading')}</h3>
									<AudioRecorder
										attemptId={attemptId}
										questionId={topic.part2_cue_card.id}
										onAudioRecorded={(url, transcript, duration, audioStoragePath) =>
											handleAudioRecorded(topic.part2_cue_card.id, 'part2', url, transcript, duration, audioStoragePath)
										}
					...
					<div className='space-y-6'>
						<div className='bg-indigo-50/80 border border-indigo-200 rounded-2xl p-6 flex items-center justify-between'>
							<div>
								...
								<h2 className='text-xl font-bold text-slate-900'>{t('ielts.test.part3Heading')}</h2>
								<p className='text-sm text-slate-600 mt-1'>{t('ielts.test.part3Instructions')}</p>
							</div>
						...
						<div className='bg-white border border-slate-200 rounded-2xl p-6 md:p-8 space-y-6 shadow-sm'>
							<div className='flex items-start justify-between gap-4'>
								<div>
									...
									<h3 className='text-xl font-extrabold text-slate-900 mt-1'>{currentP3Question.question_text}</h3>
								</div>
							...
							<AudioRecorder
								...
								onAudioRecorded={(url, transcript, duration, audioStoragePath) =>
									handleAudioRecorded(currentP3Question.id, 'part3', url, transcript, duration, audioStoragePath)
								}
						...
						<div className='flex justify-end pt-4'>
							{part3Index < topic.part3_questions.length - 1 ? (
								...
								>
									...
									<span>{submitting ? t('ielts.test.evaluatingButton') : t('ielts.test.submitButton')}</span>
								</button>
							)}
						</div>
					</div>
