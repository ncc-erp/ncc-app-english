  ...
  const fetchResult = async () => {
    try {
      ...
      if (data.success && data.result) {
        ...
        setErrorMsg(data.error || t('exam.result.loadError'));
      }
    } catch {
      setErrorMsg(t('exam.result.networkError'));
    } finally {
      setIsLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-center space-y-3">
          ...
          <p className="text-slate-600 text-sm font-medium">{t('exam.result.calculating')}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl p-8 border border-slate-200 shadow-xl max-w-md w-full text-center space-y-4">
          <h2 className="text-xl font-bold text-slate-900">{t('exam.result.resultErrorTitle')}</h2>
          <p className="text-slate-600 text-sm">{errorMsg || t('exam.result.resultNotFound')}</p>
          <button
            onClick={() => router.push('/exam')}
            className="w-full py-3 rounded-xl bg-indigo-600 text-white font-bold text-sm"
          >
            {t('exam.result.backToIntro')}
          </button>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <main className="flex-1 max-w-3xl mx-auto px-4 py-8 space-y-6 w-full">
        ...
        <PartialScoreView
          ...
          levelTitle={result.level_title || t('exam.result.defaultLevelTitle')}
          levelDescription={result.level_description || t('exam.result.defaultLevelDescription')}
        ...
        <div className="text-center pt-4">
          ...
          >
            ...
            <span>{t('exam.result.retake')}</span>
          </button>
        </div>
      </main>
    </div>
