...
export async function GET(
  _req: NextRequest,
  segmentData: { params: Promise<{ studentId: string }> }
) {
  try {
    ...
    if (!isAdmin) {
      return NextResponse.json(
        { success: false, error: 'Forbidden. Admin role in clan required.' },
        { status: 403 }
      );
    }
    ...
    for (const attempt of attempts) {
      const processResponse = (res: any, qId: string) => {
        ...
        if (!path && res.audio_url) {
          const match = res.audio_url.match(
      ...
      if (
        attempt.score_result?.responses &&
        typeof attempt.score_result.responses === 'object'
      ) {
        for (const [qId, resp] of Object.entries(
          attempt.score_result.responses as Record<string, any>,
        )) {
          processResponse(resp, qId);
        }
      }
    ...
    const completedAttempts = attempts.filter(
      (a) => a.band_score !== undefined || a.score_result !== undefined
    );
    ...
    const averageBand =
      bandScores.length > 0
        ? Number((bandScores.reduce((sum, b) => sum + b, 0) / bandScores.length).toFixed(1))
        : null;

    const highestBand =
      bandScores.length > 0 ? Math.max(...bandScores) : null;
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to fetch student speaking attempts' },
      { status: 500 }
    );
  }
}
