...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    let attempts = await pgDb.getRecentIELTSAttempts(user.user_id, limit);
    if (attempts.length === 0 && user.user_id !== user.mezon_id) {
      attempts = await pgDb.getRecentIELTSAttempts(user.mezon_id, limit);
    }
    ...
    const mappedAttempts = attempts.map((attempt) => ({
      ...
      detailsUrl: `${getAppBaseUrl()}/ielts-speaking/result/${attempt.id}/details`,
    }));
    ...
    console.error("[GET /api/bot/history] Error:", error);
    return NextResponse.json(
