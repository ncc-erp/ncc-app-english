...
export async function GET(
  ...
  try {
    ...
    let attempt = await pgDb.getIELTSAttemptByIdAndUser(
      attemptId,
      user.user_id,
    );
    if (!attempt && user.user_id !== user.mezon_id) {
      attempt = await pgDb.getIELTSAttemptByIdAndUser(attemptId, user.mezon_id);
    }
    ...
    console.error("[GET /api/bot/result/[attemptId]] Error:", error);
    return NextResponse.json(
