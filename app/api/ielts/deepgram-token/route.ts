	...
	try {
		...
		if (!response.ok || !token) {
			...
			return NextResponse.json(
				{
					success: false,
					error: data.err_msg || data.message || 'Deepgram could not create a streaming session.'
				},
				{ status: 502 }
			);
		}
		...
		return NextResponse.json({ success: false, error: 'Could not create Deepgram session.' }, { status: 500 });
	}
}
