...
export async function GET(req: NextRequest) {
	...
	try {
		...
		if (!isFirstUse) {
			console.warn(`[GET /api/ielts/launch] Launch token already used (jti: ${payload.jti})`);
		...
		return NextResponse.redirect(new URL(`/ielts-speaking/test/${payload.attemptId}`, baseUrl));
	} catch (error) {
