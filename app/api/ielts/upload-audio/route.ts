...
export async function POST(request: NextRequest) {
	...
	try {
		...
		if (!(file instanceof File) || !attemptId || !questionId) {
			return NextResponse.json({ success: false, error: 'Missing audio, attemptId, or questionId' }, { status: 400 });
		}
		...
		await uploadAudio(path, await file.arrayBuffer(), file.type || 'audio/webm');
