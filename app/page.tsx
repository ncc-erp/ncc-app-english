...
const FEATURE_COLORS = ['bg-indigo-100 text-indigo-700', 'bg-emerald-100 text-emerald-700', 'bg-rose-100 text-rose-700', 'bg-sky-100 text-sky-700'];
...
export default function LandingPage() {
	...
	const PARTS = useMemo(() => tList<DescTime>('landing.parts').map((p, i) => ({ ...p, name: PART_NAMES[i] })), [tList]);
	...
	const STEPS = useMemo(() => tList<TitleDesc>('landing.steps').map((s, i) => ({ ...s, Icon: STEP_ICONS[i] })), [tList]);
	...
	return (
		<div className='min-h-screen bg-white text-slate-900 flex flex-col font-sans'>
			...
			<main id='top' className='flex-1 w-full'>
				...
				<section className='relative overflow-hidden bg-gradient-to-br from-purple-700 via-indigo-700 to-violet-900 text-white'>
					...
					<div className='relative max-w-6xl mx-auto px-4 pt-14 pb-20 md:pt-20 md:pb-28 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center'>
						<div className='space-y-6'>
							...
							<p className='text-purple-100 text-base sm:text-lg leading-relaxed max-w-xl'>{t('landing.heroSubtitle')}</p>
						...
						<div className='relative mx-auto w-full max-w-md'>
							...
							<div className='bg-white text-slate-900 rounded-3xl p-6 md:p-8 shadow-2xl shadow-indigo-950/50 space-y-6'>
								<div className='flex items-center justify-between'>
									<div>
										<div className='text-xs font-bold text-slate-500 uppercase tracking-widest'>{t('landing.overallBandLabel')}</div>
										<div className='text-5xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-purple-700 to-indigo-700'>7.0</div>
									</div>
				...
				<section id='features' className='max-w-6xl mx-auto px-4 py-20 space-y-10 scroll-mt-20'>
					<div className='text-center space-y-3'>
						<span className='text-xs font-bold text-purple-600 uppercase tracking-widest'>{t('landing.featuresSection.eyebrow')}</span>
				...
				<section id='how' className='bg-slate-50 border-y border-slate-200 scroll-mt-20'>
					<div className='max-w-6xl mx-auto px-4 py-20 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center'>
						<div className='space-y-10'>
							<div className='space-y-3'>
								<span className='text-xs font-bold text-purple-600 uppercase tracking-widest'>{t('landing.howSection.eyebrow')}</span>
						...
						<div className='relative mx-auto w-full max-w-md'>
							...
							<div className='relative bg-white border border-slate-200 rounded-3xl p-6 md:p-8 shadow-xl space-y-6'>
								<div className='flex items-center justify-between text-xs font-bold uppercase tracking-widest'>
									<span className='px-3 py-1 rounded-full bg-purple-100 text-purple-700'>{t('landing.mockRecording.partLabel')}</span>
				...
				<section id='teacher' className='max-w-6xl mx-auto px-4 py-20 scroll-mt-20 grid grid-cols-1 lg:grid-cols-5 gap-8 items-stretch'>
					<div className='lg:col-span-3 relative overflow-hidden bg-gradient-to-br from-slate-900 to-indigo-950 text-white rounded-3xl p-8 md:p-10 flex flex-col sm:flex-row items-center gap-8'>
						...
						<div className='relative space-y-3 text-center sm:text-left'>
							<span className='text-xs font-bold text-amber-300 uppercase tracking-widest'>{t('landing.teacherSection.eyebrow')}</span>
			...
			<footer className='border-t border-slate-200 bg-white py-6'>
				<div className='max-w-6xl mx-auto px-4 text-center text-xs text-slate-500'>{t('landing.footer', { year: new Date().getFullYear() })}</div>
			</footer>
		</div>
	);
}
