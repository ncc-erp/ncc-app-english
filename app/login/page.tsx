...
function LoginContent() {
	...
	const oauthErrorMessages: Record<string, string> = {
		no_code: t('login.oauthErrors.no_code'),
		state_mismatch: t('login.oauthErrors.state_mismatch'),
		no_token: t('login.oauthErrors.no_token'),
		auth_failed: t('login.oauthErrors.auth_failed')
	};
	...
	const handlePasswordLogin = async (e: React.FormEvent) => {
		...
		if (!username || !password) {
			setFormError(t('login.errors.missingFields'));
			return;
		}
		...
		try {
			...
			if (!res.ok || !data.success) {
				setFormError(data.error || t('login.errors.loginFailed'));
				setLoading(false);
				return;
			}
			...
			setFormError(t('login.errors.serverError'));
			setLoading(false);
		}
	};
		...
		<div className='min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-4 relative overflow-hidden'>
			...
			<div className='bg-slate-900/80 backdrop-blur-xl rounded-3xl border border-slate-800 shadow-2xl max-w-md w-full p-8 text-center space-y-6 relative z-10'>
				...
				<div className='space-y-2'>
					...
					<p className='text-slate-400 text-sm'>{showPasswordForm ? t('login.subtitlePasswordForm') : t('login.subtitleDefault')}</p>
				</div>
				...
				{oauthError && !showPasswordForm && (
					<div className='p-3.5 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-xs flex items-start space-x-2 text-left'>
						...
						<span>{oauthErrorMessages[oauthError] || t('login.oauthErrors.default')}</span>
					</div>
				)}
				...
				{!showPasswordForm ? (
					...
					<div className='space-y-3 pt-2'>
						...
						>
							<span>{t('login.loginWithMezonOAuth')}</span>
						...
						>
							...
							<span>{t('login.signInWithPassword')}</span>
						</button>
					</div>
				) : (
					...
					<form onSubmit={handlePasswordLogin} className='space-y-4 pt-1 text-left'>
						...
						<div className='space-y-1.5'>
							<label className='text-xs font-semibold text-slate-300 block'>{t('login.usernameLabel')}</label>
						...
						<div className='space-y-1.5'>
							<label className='text-xs font-semibold text-slate-300 block'>{t('login.passwordLabel')}</label>
							<div className='relative'>
								<div className='absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-500'>
...
function LoginFallback() {
	...
	return <div className='min-h-screen bg-slate-950 flex items-center justify-center text-slate-400 text-sm'>{t('login.loadingPage')}</div>;
}
