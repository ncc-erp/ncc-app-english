				...
				<div className='grid grid-cols-2 md:grid-cols-4 gap-4'>
					...
					<div className='bg-white border border-slate-200 rounded-2xl p-4 flex items-center gap-3 shadow-sm'>
						...
						<div>
							...
							<div className='text-xl font-extrabold text-slate-900'>{topics.reduce((acc, t) => acc + (t.part1_questions?.length || 0), 0)}</div>
						</div>
					</div>
					...
					<div className='bg-white border border-slate-200 rounded-2xl p-4 flex items-center gap-3 shadow-sm'>
						...
						<div>
							...
							<div className='text-xl font-extrabold text-slate-900'>{topics.reduce((acc, t) => acc + (t.part3_questions?.length || 0), 0)}</div>
						</div>
					</div>
				</div>
								...
								<div className='space-y-3'>
									...
									{t.description && (
										<div className='bg-slate-50 p-3 rounded-2xl border border-slate-100 overflow-hidden'>
											<p className='text-xs text-slate-600 line-clamp-2 leading-relaxed overflow-hidden text-ellipsis'>{t.description}</p>
										</div>
									)}
								</div>
					...
					<div className='bg-white border border-slate-200 rounded-3xl p-6 md:p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto space-y-6 shadow-2xl text-slate-900'>
						<div className='flex items-center justify-between border-b border-slate-100 pb-4'>
							<div className='flex items-center space-x-3'>
								...
								<div>
									<h2 className='text-lg font-bold text-slate-900'>{editingTopic ? 'Edit IELTS Test Set' : 'Create New IELTS Test Set'}</h2>
									<p className='text-xs text-slate-500'>Specify Title, Category, and Questions for Part 1, 2 & 3.</p>
								</div>
							</div>

							<button onClick={() => setIsCreateModalOpen(false)} className='p-2 text-slate-400 hover:text-slate-700 rounded-xl hover:bg-slate-100'>
								<X className='w-5 h-5' />
							</button>
						</div>
