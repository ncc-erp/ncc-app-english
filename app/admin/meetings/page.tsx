							...
							.map((meeting) => (
								<article key={meeting.id} className='rounded-3xl border border-slate-200 bg-white p-6 shadow-sm'>
									...
									<div className='mt-4 flex flex-wrap items-center gap-2'>
										{meeting.participant_count > 0 && <span className='text-xs font-bold text-purple-700'>{t('meeting.participant')}:</span>}
									...
									<div className='space-y-2'>
										...
										{selected.created_by_name && (
											<p className='inline-flex items-center gap-1.5 text-sm font-bold text-amber-700'>
												<ShieldCheck className='h-4 w-4' />
												{t('meeting.admin')}: {selected.created_by_name}
											</p>
										)}
											...
											<div className='overflow-x-auto border-t border-slate-200'>
												<table className='w-full text-left text-sm'>
													<thead className='bg-slate-50 text-xs font-bold uppercase tracking-wide text-slate-500'>
														<tr>
															<th className='w-16 px-5 py-3'>{t('meeting.rowNumber')}</th>
															<th className='px-4 py-3'>{t('meeting.participant')}</th>
															<th className='px-4 py-3'>{t('meeting.role')}</th>
														</tr>
													</thead>
													<tbody className='divide-y divide-slate-100'>
														{selected.participants.map((p, index) => (
															<tr className='bg-white text-slate-700' key={p.mezon_id}>
																...
																<td className='px-4 py-3 text-slate-500'>{p.role ? t(`meeting.${p.role}`) : '—'}</td>
															</tr>
														))}
													</tbody>
												</table>
