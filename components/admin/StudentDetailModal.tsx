...
function extractAttemptQuestions(att: IELTSSpeakingAttempt): QuestionReviewItem[] {
  ...
  const getAnalysis = (qid: string, idx: number): IELTSPerQuestionAnalysis | undefined => {
    ...
    if (Array.isArray(perQuestion)) {
      return (
        (perQuestion as any[]).find((p: any) => p?.question_id === qid) ||
        (perQuestion as any[])[idx]
      );
    }
  ...
  if (items.length === 0) {
    const allKeys = Array.from(
      new Set([
        ...
        ...(perQuestion && !Array.isArray(perQuestion) ? Object.keys(perQuestion) : []),
      ])
    );
...
const AttemptQuestionViewer: React.FC<{ questions: QuestionReviewItem[] }> = ({
  questions,
}) => {
    ...
    <div className="space-y-3 pt-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="text-[11px] font-bold text-slate-800 uppercase tracking-wider">
              Question-by-Question Evaluation & Audio Playback
            </div>
      ...
      <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm space-y-4 text-xs transition-all">
        ...
        <div className="space-y-1.5">
          ...
          <div className="text-sm font-bold text-slate-900 pt-0.5 leading-snug">
            {qItem.questionText}
          </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex items-center gap-2 text-xs font-bold text-purple-900 shrink-0">
                ...
                {qItem.response?.duration_seconds ? (
                  <span className="text-[11px] text-purple-700 font-mono font-normal">
                    ({Math.round(qItem.response.duration_seconds)}s)
                  </span>
                ) : null}
              </div>
        ...
        {/* Student Transcript */}
        {(qItem.response?.transcript ||
          qItem.analysis?.live_stt_transcript ||
          qItem.analysis?.ai_generated_transcript) && (
        ...
        {/* Grammar Corrections */}
        {qItem.analysis?.grammar_corrections &&
          qItem.analysis.grammar_corrections.length > 0 && (
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-lg font-bold text-slate-900">
                  {student?.display_name || student?.username || 'Student Details'}
                </h2>
                ...
                <div className="bg-slate-50 border border-slate-200/80 rounded-2xl p-4 flex items-center gap-3 shadow-sm">
                  ...
                  <div>
                    ...
                    <div className="text-xl font-extrabold text-slate-900">
                      {stats?.total_speaking_attempts || 0}
                    </div>
                  </div>
                </div>
                ...
                {attempts.length === 0 ? (
                  <div className="py-12 text-center bg-slate-50 border border-dashed border-slate-200 rounded-3xl p-6 space-y-2">
                    ...
                    <p className="text-[11px] text-slate-500 max-w-sm mx-auto">
                      When this student completes an IELTS Speaking mock test on the platform or through the Mezon bot, their scores and detailed examiner feedback will appear here.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-3.5">
                    {attempts.map((att, index) => {
                      ...
                      const overallFeedback =
                        att.score_result?.summary_feedback ||
                        (att.score_result as any)?.general_feedback ||
                        '';
                        ...
                        >
                          ...
                          >
                            ...
                            <div className="flex items-center justify-between sm:justify-end gap-3 shrink-0">
                              <div
                                className={`px-3 py-1.5 rounded-xl border font-black text-xs ${getBandBadgeColor(
                                  band
                                )}`}
                              >
                          ...
                          {isExpanded && (
                            <div className="px-5 pb-5 pt-3 border-t border-slate-100 space-y-6 bg-slate-50/40">
                              ...
                              {questionItems.length > 0 && (
                                <AttemptQuestionViewer questions={questionItems} />
                              )}
                            </div>
                          )}
                        </div>
