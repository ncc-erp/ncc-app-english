 ...
 */
export function createLaunchToken(payload: Omit<LaunchTokenPayload, 'exp' | 'jti'>, expiresInMinutes = 120): string {
...
export function verifyLaunchToken(token: string): LaunchTokenPayload | null {
	try {
		...
		if (sig.length === expectedSig.length && crypto.timingSafeEqual(Buffer.from(sig), Buffer.from(expectedSig))) {
			const payload: LaunchTokenPayload = JSON.parse(Buffer.from(data, 'base64url').toString('utf8'));
