...
function buildResultCommandOutput(attempt: IELTSSpeakingAttempt, user: any, mezonUserId: string): CommandResult {
	const baseUrl = getAppBaseUrl();
	const detailsUrl = `${baseUrl}/ielts-speaking/result/${attempt.id}/details`;
	const formattedText = formatIELTSResult(attempt, user.display_name || user.mezon_username, detailsUrl);
 ...
 */
export async function handleResultCommand(mezonUserId: string, args: string[] = []): Promise<CommandResult> {
	...
	if (specificAttemptId) {
		const attempt = await pgDb.getIELTSAttemptByIdAndUser(specificAttemptId, user.user_id);
		...
		if (!attempt) {
			...
			if (directAttempt && (directAttempt.user_id === user.user_id || directAttempt.user_id === user.mezon_id)) {
 ...
 */
export async function handleHistoryCommand(mezonUserId: string): Promise<CommandResult> {
	const user = await pgDb.getUserByMezonId(mezonUserId);
	if (!user) {
		return {
	...
	return {
		text: formatIELTSTestHistory(attempts, user.display_name || user.mezon_username),
		isSuccess: true
	};
}
...
export async function handleTestingNowCommand(
	...
	if (query) {
		const matched = topics.find(
			(t) => t.id.toLowerCase() === query || t.title.toLowerCase().includes(query) || t.category?.toLowerCase().includes(query)
		);
		if (matched) {
