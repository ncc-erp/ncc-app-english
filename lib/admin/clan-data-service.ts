...
function isAdminRole(title?: string): boolean {
  ...
  return (
    norm === "admin" || norm === "administrator" || norm === "quan tri vien"
  );
}
...
function matchChannelAndRole(channelName: string, roleTitle: string): boolean {
  ...
  const normChan = normalizeText(channelName)
    .replace(/[-_]/g, " ")
    .replace(/\s+/g, " ");
  const normRole = normalizeText(roleTitle)
    .replace(/[-_]/g, " ")
    .replace(/\s+/g, " ");
  ...
  const cleanRole = normRole
    .replace(/^(lop|class|role|hoc vien|sinh vien)\s+/, "")
    .trim();

  if (cleanChan.length >= 3 && cleanRole.length >= 3) {
    if (
      cleanChan === cleanRole ||
      cleanChan.includes(cleanRole) ||
      cleanRole.includes(cleanChan)
    ) {
      return true;
    }
  }
  ...
  if (verifyUrl) {
    try {
      const res = await fetch(
        `${verifyUrl.replace(/\/$/, "")}/verify-admin?userId=${encodeURIComponent(mezonUserId)}`,
 ...
 */
export async function getClanClassrooms(
    ...
    if (client && clanId) {
      ...
      if (targetClan) {
        if (forceRefresh || !(targetClan as any)._channelsLoaded) {
          ...
          const loadAction =
            typeof (targetClan as any).reloadChannels === "function"
              ? (targetClan as any).reloadChannels()
              : targetClan.loadChannels();
...
function safeDecodeChannelUserList(bytes: Uint8Array): ChannelUserList {
  try {
    ...
    console.warn(
      "[Clan Data Service] Rescued ChannelUserList decode error:",
      err?.message,
    );
    try {
...
async function scanChannelMembers(
  ...
  while (page < maxPages) {
    ...
    try {
      ...
      const requestBody = ListChannelUsersRequest.encode(
        ListChannelUsersRequest.fromPartial(requestPayload),
      ).finish();
      ...
      const invokePromise = internalClient.apiClient.invokeMezonApi(
        "/mezon.api.Mezon/ListChannelUsers",
        requestBody,
        {
      ...
      if (
        !res ||
        !Array.isArray(res.channel_users) ||
        res.channel_users.length === 0
      ) {
        break;
      }
      ...
      console.warn(
        `[Clan Data Service] Failed to scan members for channel ${channel.name || channelId}:`,
        err,
      );
      break;
    }
 ...
 */
export async function getClanStudents(
  classId?: string,
  forceRefresh: boolean = false,
): Promise<StudentData[]> {
  ...
  try {
    ...
    if (client && clanId) {
      ...
      if (targetClan) {
        ...
        if (
          forceRefresh ||
          !(targetClan as any)._channelsLoaded ||
          targetClan.channels.size === 0
        ) {
          try {
            (targetClan as any)._channelsLoaded = false;
            let timer: NodeJS.Timeout | undefined;
            ...
            console.warn(
              "[Clan Data Service] Failed to load clan channels:",
              loadErr,
            );
          }
        }
        ...
        const classroomChannels = Array.from(
          targetClan.channels.values(),
        ).filter((ch) => isLopHocCategory(ch.category_name));
        ...
        const channelScanPromises = classroomChannels.map((ch) =>
          scanChannelMembers(internalClient, clanId, ch),
        );
        ...
        const usersTimeout = new Promise<ClanUserList>((resolve) => {
          usersTimer = setTimeout(() => {
            console.warn(
              `[Clan Data Service] ListClanUsers timed out after 10000ms for clan ${clanId}`,
            );
        ...
        for (const entry of clanUsersRes.clan_users) {
          ...
          if (user.id === client.clientId || (user as any).bot === true)
            continue;
          ...
          const userRoleIds: string[] = [
            ...
            ...(Array.isArray((entry as any).role_ids)
              ? (entry as any).role_ids
              : []),
            ...(Array.isArray((entry as any).roles)
              ? (entry as any).roles.map((r: any) => r?.id || r)
              : []),
          ].map(String);
          ...
          if (isStudent) {
            ...
            for (const ch of classroomChannels) {
              ...
              const isPrivate = scanData
                ? scanData.isPrivate
                : Boolean(ch.is_private);
              ...
              if (!belongs && scanData && scanData.memberRoleIds.size > 0) {
                if (
                  userRoleIds.some((rid) => scanData.memberRoleIds.has(rid))
                ) {
                  belongs = true;
                }
              }
              ...
              if (!belongs) {
                const roleMatched = userRoleTitles.some((title) =>
                  matchChannelAndRole(ch.name || "", title),
                );
                if (roleMatched) {
                  belongs = true;
                }
              ...
              if (
                !belongs &&
                !isPrivate &&
                (!scanData || scanData.memberUserIds.size === 0)
              ) {
                const anyRoleMatchesChannel = roles.some((r) =>
                  matchChannelAndRole(ch.name || "", r.title || ""),
                );
          ...
          for (const [userId, uInfo] of scanData.users.entries()) {
            ...
            if (existing) {
              ...
              studentsMap.set(userId, {
                mezon_id: userId,
                username:
                  cachedUser?.username || uInfo.clanNick || `user_${userId}`,
                display_name:
                  uInfo.clanNick ||
                  cachedUser?.display_name ||
                  cachedUser?.username ||
                  `Student_${userId.slice(-4)}`,
